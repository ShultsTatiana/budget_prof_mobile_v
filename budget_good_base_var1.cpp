...
struct BulkTaxApplier {
    size_t tax = TAX_PERCENTAGE;
    uint32_t count = 0;
    double FACTOR = 1.0 - tax / 100.0;

    double ComputeFactor() const {
        return pow(FACTOR, count);
    }
};
//Добавить обновление процента налога
//////////////////////////////////////////////////////////////////////////////////////////////
...
public:
    ...
    BulkLinearUpdater(const BulkTaxApplier& tax)
        : tax_(tax)
    {}
    BulkLinearUpdater(const BulkMoneySubtractor& sub)
        : sub_(sub)
    {}
    ...
    double Collapse(double origin, IndexSegment segment) const {
        double x1 = origin * tax_.ComputeFactor();
        double x2 = add_.delta * segment.length();
        double x3 = sub_.delta * segment.length();
        return x1 + x2 - x3;
    }
...
private:
    ...
    BulkTaxApplier tax_;
    BulkMoneyAdder add_;
    BulkMoneySubtractor sub_;
};
////////////////////////////////////////////////////////////////////////////////////////////
//manager.AddBulkOperation(date_segment, BulkMoneyAdder{ daily_income });
//manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier{ tax, 1 });
//manager.ComputeSum(MakeDateSegment(date_from, date_to));
...
private:
    ...
    static NodeHolder Build(IndexSegment segment) {
        if (segment.empty()) {
            ...
            return make_unique<Node>(Node{
              Build({segment.left, middle}), //left
              Build({middle, segment.right}), //right
              segment, //segment
                });
        }
    }
...
static const Date START_DATE = Date::FromString("2000-01-01");
static const Date END_DATE = Date::FromString("2000-01-30");
...
struct SpendRequest : ModifyRequest {
    ...
    // проверка правки относительно структуры manager метода AddBulkOperation
    void Process(BudgetManager& manager) const override {
        const auto date_segment = MakeDateSegment(date_from, date_to);
...
/*void testParseQuery() {
    stringstream input{ "Earn \nComputeIncome \nPayTax \nSpend \nEarn \n" };
    IncomeOperator incom;
    ASSERT_EQUAL(static_cast<int>(incom.readQuery(input)), static_cast<int>(IncomeQuery::EARN));
    ASSERT_EQUAL(static_cast<int>(incom.readQuery(input)), static_cast<int>(IncomeQuery::COMPUTEINCOME));
    ASSERT_EQUAL(static_cast<int>(incom.readQuery(input)), static_cast<int>(IncomeQuery::PAYTAX));
    ASSERT_EQUAL(static_cast<int>(incom.readQuery(input)), static_cast<int>(IncomeQuery::SPEND));
    ASSERT_EQUAL(static_cast<int>(incom.readQuery(input)), static_cast<int>(IncomeQuery::EARN));
}

void testReadDate() {
    stringstream input{ "2000 - 01 - 02 2010 - 02 - 06 \n" };

    Date expact1 = ParserDate().setYear(2000).setMonth(1).setDay(2).Build();
    Date expact2 = ParserDate().setYear(2010).setMonth(2).setDay(6).Build();
    Date out1 = ParserDate().readDate(input);
    Date out2 = ParserDate().readDate(input);

    ASSERT_EQUAL(out1, expact1);
    ASSERT_EQUAL(out2, expact2);

}

void testParseIncome() {
    stringstream input{ "4\n\
Earn 2000 - 01 - 02 2000 - 01 - 06 20\n\
ComputeIncome 2000 - 01 - 01 2001 - 01 - 01\n\
PayTax 2000 - 01 - 02 2000 - 01 - 03 13\n\
ComputeIncome 2000 - 01 - 01 2001 - 01 - 01\n\
Spend 2000 - 01 - 02 2000 - 01 - 03\n"};
    IncomeOperator income;
    int Q(0);
    input >> Q;
    vector <Query> expect {
        {IncomeQuery::EARN,
         ParserDate().setYear(2000).setMonth(1).setDay(2).Build(),
         ParserDate().setYear(2000).setMonth(1).setDay(6).Build(),
         20
        },
        {IncomeQuery::COMPUTEINCOME,
        ParserDate().setYear(2000).setMonth(1).setDay(1).Build(),
        ParserDate().setYear(2001).setMonth(1).setDay(1).Build()},
        {IncomeQuery::PAYTAX,
        ParserDate().setYear(2000).setMonth(1).setDay(2).Build(),
        ParserDate().setYear(2000).setMonth(1).setDay(3).Build(),
        13
        },
        {IncomeQuery::COMPUTEINCOME,
        ParserDate().setYear(2000).setMonth(1).setDay(1).Build(),
    ...
    for (int i(0); i < Q; ++i) {
        ASSERT_EQUAL(income.readIncome(input), expect[i]);
    }
}*/
...
Earn 2000-01-02 2000-01-06 20\n\
ComputeIncome 2000-01-01 2000-01-30\n\
PayTax 2000-01-02 2000-01-03 20\n\
ComputeIncome 2000-01-01 2000-01-30\n\
Spend 2000-01-20 2000-01-23 14\n\
ComputeIncome 2000-01-01 2000-01-30\n\
PayTax 2000-01-20 2000-01-20 13\n\
ComputeIncome 2000-01-01 2000-01-30\n" };
    vector<optional<double>> expectResult{
        nullopt,
        20.0,
        nullopt,
        4.0 * 0.8 + 4.0 * 0.8 + 12.0,
        nullopt,
        4.0 * 0.8 + 4.0 * 0.8 + 12.0 - 3.5 - 3.5 - 3.5 - 3.5,
        nullopt,
        4.0 * 0.8 + 4.0 * 0.8 + 12.0 - 3.5 - 3.5 - 3.5 - 3.5
    };
    stringstream output;
...
ComputeIncome 2000-01-01 2001-01-01\n" };
    vector<optional<double>> expectResult{
        ...
        4.0 * 0.8 + 4.0 * 0.8 + 12.0,
        nullopt,
        4.0 * 0.8 + 4.0 * 0.8 + 12.0 - 3.5 - 3.5 - 3.5,
        nullopt,
        4.0 * 0.8 + 4.0 * 0.8 + 12.0 - 3.5 - 3.5 - 3.5
    };
    stringstream output;
...
Earn 2099-12-31 2099-12-31 20\n\
ComputeIncome 2099-12-31 2099-12-31\n\
ComputeIncome 2000-01-01 2099-12-31\n\
PayTax 2000-01-01 2099-12-31 13\n\
ComputeIncome 2000-01-01 2099-12-31\n\
...
ComputeIncome 2000-01-01 2099-12-31\n" };
    vector<optional<double>> expectResult{
        ...
        20.0 * 0.87 + 20.0 * 0.87,
        nullopt,
        (20.0 * 0.87 + 1.0) + 36523.0 + (20.0 * 0.87 + 1.0),
        nullopt,
        (20.0 * 0.87 + 1.0) * 0.87 + 36523.0 * 0.87 + (20.0 * 0.87 + 1.0) * 0.87
    };
    stringstream output;
    cout.precision(25);
    const auto requests = ReadRequests(input);
    ...
    //cout << 20.0 << '\n' << 20.0 << '\n'
      //  << 40.0 << '\n' << 34.8 << '\n'
        //<< 36559.8 << '\n' << 31807.026 << '\n';
    for (const optional<double>& i : expectResult) {
        if (i) {
            cout << *i << " ";
        }
    }
    cout << "\n" << output.str();
    //ASSERT_EQUAL(output.str(), expect.str());
}
...
int main() {
    ...
    //RUN_TEST(tr, testParseIncome);
    RUN_TEST(tr, verySimpleTest);
    //RUN_TEST(tr, simpleTest);
    //RUN_TEST(tr, testBoundary);
