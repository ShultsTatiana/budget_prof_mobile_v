...
struct Money {
    ...
    Money operator*(Data factor) const {
        return { earn * factor, spend * factor };
    }
    operator Data() const {
        return earn - spend;
    } 
...
public:
    ...
    void CombineWith(const BulkLinearUpdater& other) {
        ...
        add_.delta.earn = add_.delta.earn * other.tax_.FACTOR + other.add_.delta.earn;
        add_.delta.spend += other.add_.delta.spend;
    }

    Money Collapse(const Money& origin, IndexSegment segment) const {
        return Money{ origin.earn * tax_.FACTOR, origin.spend } + add_.delta * double(segment.length());
    }
...
struct AddMoneyRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        ...
        const Money daily_change = (
            SIGN == 1 ? 
            Money{ .earn = daily_value } : Money{ .spend = daily_value }
        );
...
RequestHolder ParseRequest(string_view request_str) {
    ...
    //создается запрос (2-х видов ModifyRequest и ReadRequest)
    RequestHolder request = Request::Create(*request_type); //RequestHolder = unique_ptr<Request>
    if (request) {
...
//RequestHolder = unique_ptr<Request>
...
//читает вектор запросов, обрабатывает каждый из них
vector<double> ProcessRequests(const vector<RequestHolder>& requests) {
    vector<double> responses;
    BudgetManager manager; //создал менеджер бюджета
    for (const auto& request_holder : requests) {  //пощел по запросам
        //запросы бывают 2-х видов: на расчет(ComputeIncome) и на изменение бюджета (Earn, PayTax, Spend)
...
//просто записывает вектор ответов
